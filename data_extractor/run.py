...
def get_parser():
    ...
    parser.add_argument('--datadir', type=str, required=True,
                        help='Data directory path to store bulletins and database')
    parser.add_argument('--run_only', type=str, required=False, default=None,
                        help='Comma-separated values of states to run data extraction for')
    parser.add_argument('--force_run_states', type=str, required=False, default=None,
                        help='Comma-separated values of states to force re-run data extraction procedure for')
...
def run(args):
    ...
    if args.force_run_states is not None:
        force_rerun_states = [x.strip()
                              for x in args.force_run_states.split(',')]

    # Download bulletins
    bulletin_links = bulletin_downloader.run(
        args.datadir, state_to_execute=states_to_execute)
    ...
    for state in state_pbar:
        ...
        metadata_path = os.path.join(
            args.datadir, 'metadata', 'bulletins', f'{state}.json')
        ...
        date_pbar = tqdm(data[BULLETIN_PATH_STR].items(),
                         desc="Dates", leave=False)
